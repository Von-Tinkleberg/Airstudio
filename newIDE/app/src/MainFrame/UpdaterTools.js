...
export const getElectronUpdateStatusLabel = (
  ...
  if (status === 'update-downloaded')
    return (
      <Trans>
        A new update will be installed after you quit and relaunch AirStudio
      </Trans>
    );
  return '';
};
...
export const getElectronUpdateButtonLabel = (status: string): React.Node => {
  if (status === 'update-available')
    return <Trans>Update AirStudio to latest version</Trans>;
...
export const getServiceWorkerStatusLabel = (
  ...
  if (status === 'not-installed') {
    ...
    return (
      <Trans>
        An update is ready to be installed. Close ALL AirStudio apps or tabs in
