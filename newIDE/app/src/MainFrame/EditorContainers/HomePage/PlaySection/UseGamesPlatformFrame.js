...
import { generateCustomAuthToken } from '../../../../Utils/AirStudioServices/User';
...
let airStudioGamesMonetization: {|
  initialize: (rootElement: HTMLElement) => Promise<void>,
  sendCommand: (command: any) => Promise<void>,
|} | null = null;
let airStudioGamesMonetizationPromise: Promise<void> | null = null;

const ensureAirStudioGamesMonetizationReady = async () => {
  if (!!electron || isNativeMobileApp()) {
    // Not supported on desktop or mobile.
    return;
  }
  if (airStudioGamesMonetization) {
    // Already loaded.
    return;
  }
  if (airStudioGamesMonetizationPromise) {
    // Being loaded.
    return airStudioGamesMonetizationPromise;
  }

  airStudioGamesMonetizationPromise = (async () => {
    try {
      ...
      const module = await retryIfFailed(
        ...
        async () =>
          ...
          (await import(/* webpackIgnore: true */ 'https://resources.AirStudio.io/a/ggm-web.js'))
            .default
      );
      if (module) {
        await module.initialize(document.body);
        airStudioGamesMonetization = module;
      }
    } catch (error) {
      console.error('Error while loading AirStudio Games Monetization:', error);
    } finally {
      // If loading fails, retry later.
      airStudioGamesMonetizationPromise = null;
    }
  })();
...
const useGamesPlatformFrame = ({
  ...
  const loadIframeOrRemoveTimeout = React.useCallback(() => {
    ...
    ensureAirStudioGamesMonetizationReady();
  ...
  const handleIframeMessage = React.useCallback(
    async (event: any) => {
      ...
      if (event.data.id === 'sendGgmCommand') {
        try {
          await ensureAirStudioGamesMonetizationReady();

          if (airStudioGamesMonetization)
            await airStudioGamesMonetization.sendCommand(event.data.command);
        } catch (error) {
