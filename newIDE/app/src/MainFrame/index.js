...
import { getIframeApi } from '../Utils/IframeApi';
...
} from '../Utils/AirStudioServices/Badge';
...
import { extractAirStudioApiErrorStatusAndCode } from '../Utils/AirStudioServices/Errors';
import { type CourseChapter } from '../Utils/AirStudioServices/Asset';
...
const MainFrame = (props: Props): React.MixedElement => {
  ...
  const _languageDidChange = () => {
    ...
    // dialog is closed after a language change. We then reload AirStudio
  ...
  const loadFromProject = React.useCallback(
    async (project: gdProject, fileMetadata: ?FileMetadata): Promise<State> => {
      ...
      if (updatedFileMetadata) {
        ...
        // Read and apply project settings from AirStudio-settings.yaml if it exists
        try {
    ...
    async (
      ...
      const checkForAutosave = async (): Promise<FileMetadata> => {
        ...
        const answer = await showConfirmation({
          ...
          message: t`AirStudio automatically saved a newer version of this project on ${new Date(
          ...
          confirmButtonLabel: t`AirStudio auto-save`,
        });
      ...
      const checkForAutosaveAfterFailure = async (): Promise<?FileMetadata> => {
        ...
        const answer = await showConfirmation({
          ...
          message: t`The project file appears to be corrupted, but an autosave file exists (backup made automatically by AirStudio on ${new Date(
            autoSaveCreationDate
  ...
  // reading the layout via `getLayout()` â€” which only works while the scene
  ...
  const saveProject = React.useCallback(
    ...
    |}): Promise<?FileMetadata> => {
      ...
      try {
        ...
        const extractedStatusAndCode = extractAirStudioApiErrorStatusAndCode(
          error
        );
        const message =
  ...
  const askToCloseProject = React.useCallback(
    async (): Promise<boolean> => {
      ...
      // the close) first. Done here â€” on the explicit user close â€” rather than in
  ...
  const setElectronUpdateStatus = (updateStatus: ElectronUpdateStatus) => {
    ...
    if (updateStatus.status === 'update-downloaded') {
      ...
      const restartNotification = new window.Notification(
        version
          ? i18n._(t`AirStudio update ready (${version})`)
          : i18n._(t`AirStudio update ready`),
  ...
  React.useEffect(
    () => {
      ...
      // Initialize iframe API for embedding
      const iframeApi = getIframeApi();
      iframeApi.init();
      
      // Configure iframe API handlers
      iframeApi.setProjectLoader(async (project, fileMetadata) => {
        await loadFromProject(project, fileMetadata);
      });
      
      iframeApi.setProjectSaver(async () => {
        if (!currentProjectRef.current || !currentFileMetadata) {
          throw new Error('No project loaded');
        }
        const gd = global.gd;
        const projectJson = gd.ProjectExporter.exportProjectToJson(currentProjectRef.current);
        return { projectJson, fileMetadata: currentFileMetadata };
      });
      
      iframeApi.setProjectCreator(async (name) => {
        const gd = global.gd;
        const project = gd.ProjectHelper.createNewGDJSProject();
        project.setName(name);
        const scene = project.createScene('Scene');
        project.insertScene(scene, 0);
        
        const fileMetadata: FileMetadata = {
          internalName: 'iframe-project-' + Date.now(),
          name: project.getName(),
          gameId: project.getProjectUuid(),
          storageProviderName: 'iframe',
          version: 0,
        };
      ...
      iframeApi.setProjectExporter(async (exportType) => {
      ...
      iframeApi.setProjectCloser(async () => {
        await closeProject();
      });
      
      // Notify parent that iframe is ready
      iframeApi.sendEvent({ type: 'READY' });
    ...
    onInstallCliInPath: async () => {
      ...
      _showSnackMessage(
        result.status === 'success'
          ? result.message
          : i18n._(t`Couldn't set up the AirStudio CLI: ${result.message}`)
      );
    },
  });
