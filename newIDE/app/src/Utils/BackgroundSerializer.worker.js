...
let modulePromise /*: ?Promise<libAirStudio>*/ = null;

const log = (message /*: string */) => {
  console.log(`[BackgroundSerializerWorker] ${message}`);
};

const getLibAirStudio= (versionWithHash /*: string */) => {
  ...
  modulePromise = new Promise((resolve, reject) => {
    try {
      ...
      if (typeof initializeAirStudioJs !== 'function') {
        /* eslint-enable no-undef */
        reject(new Error('Missing initializeAirStudioJs in worker'));
        return;
      }
...
self.onmessage = async (event /*: MessageEvent */) => {
  ...
  try {
    // $FlowFixMe[incompatible-type]
    const gd = await getLibAirStudio(versionWithHash);
