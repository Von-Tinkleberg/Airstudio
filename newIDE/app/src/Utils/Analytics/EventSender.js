...
|} | null = null;
let airStudioEditorAnalyticsPromise: Promise<void> | null = null;

const ensureAirStudioEditorAnalyticsReady = async () => {
  if (airStudioEditorAnalytics) {
    // Already loaded.
    return;
  }

  if (airStudioEditorAnalyticsPromise) {
    // Being loaded.
    return airStudioEditorAnalyticsPromise;
  }

  airStudioEditorAnalyticsPromise = (async () => {
    try {
      ...
      const module = await retryIfFailed(
        ...
        async () =>
          ...
          (await import(/* webpackIgnore: true */ 'https://resources.AirStudio.io/a/gea.js'))
            .default
      );
      if (module) {
        ...
        airStudioEditorAnalytics = module;
      }
    } catch (error) {
      console.error('Error while loading AirStudio Editor Analytics:', error);
    } finally {
      // If loading fails, retry later.
      airStudioEditorAnalyticsPromise = null;
    }
  })();
...
const recordEvent = (name: string, metadata?: { [string]: any }) => {
  ...
  (async () => {
    await ensureAirStudioEditorAnalyticsReady();
    if (airStudioEditorAnalytics) {
      await airStudioEditorAnalytics.trackEvent(name, metadata || {});
    }
  })();
};
...
export const installAnalyticsEvents = () => {
  ...
  ensureAirStudioEditorAnalyticsReady().catch(() => {
...
export const identifyUserForAnalytics = (
  ...
  const userProperties = {
    ...
    airStudioUsage: profile ? profile.airStudioUsage : undefined,
  ...
  (async () => {
    await ensureAirStudioEditorAnalyticsReady();
    if (airStudioEditorAnalytics) {
      await airStudioEditorAnalytics.identify(userId, userProperties);
    }
  })();
};
...
export const sendExportLaunched = (exportKind: string) => {
  recordEvent('export_launched', {
    platform: 'AirStudio JS Platform', // Hardcoded here for now
    exportKind,
  });
};
...
export type SubscriptionDialogDisplayReason =
  | 'Disable AirStudio splash at startup'
  | 'Debugger'
  | 'Hot reloading'
  | 'Preview over wifi'
