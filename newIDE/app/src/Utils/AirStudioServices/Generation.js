...
import { AirStudioAiCdn, AirStudioGenerationApi } from './ApiConfigs';
...
export const apiClient: Axios = axios.create({
  baseURL: AirStudioGenerationApi.baseUrl,
});
...
export const getAiRequest = async (
  ...
  const response = await axios.get(
    `${AirStudioGenerationApi.baseUrl}/ai-request/${aiRequestId}`,
    {
      params: {
        userId,
...
> => {
  ...
  // $FlowFixMe[underconstrained-implicit-instantiation]
  const response = await axios.get(
    `${AirStudioGenerationApi.baseUrl}/ai-request`,
    {
      params: {
        userId,
...
export const createAiRequest = async (
  ...
  const response = await apiClient.post(
    '/ai-request',
    {
      AirStudioVersionWithHash: getIDEVersionWithHash(),
      userRequest,
      gameProjectJson,
      gameProjectJsonUserRelativeKey,
      projectSpecificExtensionsSummaryJson,
...
|}): Promise<AiSettings> => {
  // $FlowFixMe[underconstrained-implicit-instantiation]
  const response = await axios.get(
    `${AirStudioAiCdn.baseUrl[environment]}/ai-settings-v2.json`
  );
  return ensureObjectHasProperty({
    data: response.data,
