...
import { AirStudioShopApi } from './ApiConfigs';
...
export const client: Axios = axios.create({
  baseURL: AirStudioShopApi.baseUrl,
});
...
export const isPrivateAssetResourceAuthorizedUrl = (url: string): boolean =>
  url.startsWith('https://private-assets.AirStudio.io/') ||
  url.startsWith('https://private-assets-dev.AirStudio.io/');

export const isPrivateGameTemplateResourceAuthorizedUrl = (
  url: string
): boolean =>
  url.startsWith('https://private-game-templates.AirStudio.io/') ||
  url.startsWith('https://private-game-templates-dev.AirStudio.io/');
...
|}): string => {
  const url = new URL(
    `${AirStudioShopApi.baseUrl}/purchase/action/redirect-to-checkout`
  );
...
|}): string => {
  const url = new URL(
    `${AirStudioShopApi.baseUrl}/purchase/action/redirect-to-stripe-checkout`
  );
...
|}):
  ...
  if (redeemCondition.reason === 'subscription') {
    // Condition should look like `AirStudio_gold,AirStudio_startup`.
