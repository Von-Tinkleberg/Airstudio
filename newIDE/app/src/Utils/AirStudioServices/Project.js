...
import {
  AirStudioProjectApi,
  AirStudioProjectResourcesStorage,
} from './ApiConfigs';
...
export const projectResourcesClient: Axios = axios.create({
  baseURL: AirStudioProjectResourcesStorage.baseUrl,
...
export const apiClient: Axios = axios.create({
  baseURL: AirStudioProjectApi.baseUrl,
});
...
export const storeAirStudioResourceJwtToken = (token: string) => {
  gdResourceJwt = token;
};

export const getAirStudioResourceJwtToken = (): string | null => {
  return gdResourceJwt;
};

export const cleanAirStudioResourceJwtToken = () => {
  gdResourceJwt = null;
};

export const addAirStudioResourceJwtTokenToUrl = (url: string): string => {
...
export const getCredentialsForCloudProject = async (
  ...
  if (isNativeMobileApp()) {
    if (response.data && typeof response.data.gd_resource_jwt === 'string')
      storeAirStudioResourceJwtToken(response.data.gd_resource_jwt);
  }
...
export const getCloudProjectFileMetadataIdentifier = (
  ...
  if (fileMetadata.fileIdentifier.startsWith('http')) {
    ...
    // to the example such as `https://ressources.AirStudio-app.com/...`.
  ...
  await refetchCredentialsForProjectAndRetryIfUnauthorized(
    ...
    () =>
      projectResourcesClient.post(
        addAirStudioResourceJwtTokenToUrl(presignedUrl),
        zippedProject,
        {
  ...
  await PromisePool.withConcurrency(10)
    ...
    .process(async ({ resourceFile, presignedUrl, index }) => {
      try {
        ...
        const fullUrl = `${
          AirStudioProjectResourcesStorage.baseUrl
        }${presignedUrl}`;
...
export const extractDecodedFilenameFromProjectResourceUrl = (
  ...
  if (url.startsWith(AirStudioProjectResourcesStorage.baseUrl)) {
