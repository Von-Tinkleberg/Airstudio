...
import { AirStudioUsageApi } from './ApiConfigs';
...
export const apiClient: Axios = axios.create({
  baseURL: AirStudioUsageApi.baseUrl,
});

export const canPriceBeFoundInAirStudioPrices = (
...
export const hasValidSubscriptionPlan = (
  ...
  // AirStudio: Everyone has a valid subscription (free for all)
  return true;
};
...
export const canUpgradeSubscription = (
  ...
  // AirStudio: No upgrades needed - everything is free
  return false;
};
...
|}): string => {
  const url = new URL(
    `${AirStudioUsageApi.baseUrl}/subscription-v2/action/redirect-to-checkout-v2`
  );
