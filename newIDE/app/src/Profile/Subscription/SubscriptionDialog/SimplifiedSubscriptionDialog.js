...
const DEFAULT_FEATURED_PLAN_ID = 'AirStudio_gold';
...
const colors = {
  ...
  mutedBullet: '#6E6B85',
  airStudioBadge: 'linear-gradient(135deg, #6C56F0, #8B57F0)',
};
...
const styles = {
  ...
  airStudioBadgeSquare: {
    width: 14,
    height: 14,
    borderRadius: 4,
    background: colors.airStudioBadge,
...
|}) => (
  <div style={styles.bulletLine}>
    <span style={enabled ? styles.check : styles.mutedBullet}>
      {enabled ? <CheckCircleFilled fontSize="small" /> : 'â—'}
    </span>
...
|}) => {
  ...
  const isLightTheme = airStudioTheme.palette.type === 'light';
  return (
    <span
      style={{
        ...styles.storeBadge,
...
const storeBadgesById: {
  ...
  'gd-games': {
    icon: <span style={styles.airStudioBadgeSquare} />,
    label: 'gd.games',
  },
};
          ...
          >
            <Line noMargin alignItems="baseline">
              <ColumnStackLayout noMargin>
                ...
                {yearlyPricingSystem && (
                  <Text noMargin color="secondary" size="body-small">
                    <Trans>Billed annually Â· cancel anytime</Trans>
                  </Text>
                )}
              </ColumnStackLayout>
            </Line>
              ...
              <RaisedButton
                ...
                label={
                  <LeftLoader isLoading={isLoading}>
                    {upgradeButtonLabel ? (
                      `${upgradeButtonLabel} â†’`
                    ) : (
                      <Trans>Upgrade to {featuredPlanName} â†’</Trans>
                    )}
                  </LeftLoader>
                }
