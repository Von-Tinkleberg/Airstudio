...
import { isCompatibleWithAirStudioVersion } from '../../Utils/Extension/ExtensionCompatibilityChecker.js';
...
export const openExampleInWebApp = (exampleShortHeader: ExampleShortHeader) => {
  Window.openExternalURL(
    `${
      isDev ? 'http://localhost:3000' : 'https://editor.airstudio.io'
...
const ExampleInformationPage = ({ exampleShortHeader }: Props): React.Node => {
  const isCompatible = isCompatibleWithAirStudioVersion(
    getIDEVersion(),
    exampleShortHeader.AirStudioVersion
  );
    ...
    <ColumnStackLayout expand noMargin>
      {!isCompatible && (
        <AlertMessage kind="error">
          <Trans>
            Download the latest version of AirStudio to check out this example!
          </Trans>
        </AlertMessage>
      )}
