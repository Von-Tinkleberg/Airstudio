...
import { checkIfIsAirStudioCloudBucketUrl } from '../../Utils/CrossOrigin';
import { extractDecodedFilenameFromProjectResourceUrl } from '../../Utils/AirStudioServices/Project';
import {
  archiveFiles,
      ...
      .map(
        (resourceName: string): ?ResourceToFetch => {
          ...
          if (isURL(resourceFile)) {
            if (checkIfIsAirStudioCloudBucketUrl(resourceFile)) {
              return {
                resource,
                url: resourceFile,
  ...
  return (
    <Dialog
      ...
      actions={[
        <FlatButton
          key="download"
          label={<Trans>Download AirStudio desktop version</Trans>}
          primary={false}
      ...
      <ColumnStackLayout noMargin>
        <Text>
          <Trans>
            ...
            using the full AirStudio version:
          </Trans>
        </Text>
            ...
            <BlobDownloadUrlHolder blob={zippedProjectBlob}>
              {blobDownloadUrl => (
                <RaisedButton
                  primary
                  onClick={() =>
                    openBlobDownloadUrl(blobDownloadUrl, 'AirStudio-game.zip')
                  }
                  label={
