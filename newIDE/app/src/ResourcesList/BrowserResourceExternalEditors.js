...
const externalEditorIndexHtml: { ['sprite-editor' | 'aircanvas' | 'yarn' | 'jfxr']: string } = {
  'sprite-editor': 'external/sprite-editor/index.html',
  aircanvas: 'external/aircanvas/index.html',
...
const openAndWaitForExternalEditorWindow = async ({
  ...
  externalEditorName: 'sprite-editor' | 'aircanvas' | 'yarn' | 'jfxr',
    ...
    const onMessageEvent = (event: MessageEvent) => {
      ...
      if (id === `external-editor-ready`) {
        ...
        externalEditorWindow.postMessage(
          ...
          // Ensure only external editors hosted on the same server as the AirStudio editor
...
|}) => {
  ...
  // Save the edited files back to the AirStudio resources, as "blob urls" (blob:...)
  ...
  // Some external editors need to have resource names persisted.
  patchExternalEditorMetadataWithResourcesNamesIfNecessary(
...
const immediatelyOpenLoadingWindowForExternalEditor = () => {
  const targetId = 'AirStudioExternalEditor' + nextExternalEditorWindowId++;
  const width = 800;
  const height = 600;
...
const editors: Array<ResourceExternalEditor> = [
  {
    name: 'sprite-editor',
    createDisplayName: t`Create with Sprite Editor`,
    editDisplayName: t`Edit with Sprite Editor`,
    kind: 'image',
    edit: async options => {
      ...
      return await editWithBrowserExternalEditor({
        options,
        externalEditorWindow,
        externalEditorName: 'sprite-editor',
        defaultName: 'New sprite',
        metadataKey: 'sprite',
        resourceKind: 'image',
      });
    },
  ...
  {
    name: 'aircanvas',
    createDisplayName: t`Create with AirCanvas`,
    editDisplayName: t`Edit with AirCanvas`,
    kind: 'image',
    edit: async options => {
      if (options.getStorageProvider().internalName !== 'Cloud') {
        const { i18n } = options;
