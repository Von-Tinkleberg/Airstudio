...
import { type Game } from '../../../../Utils/AirStudioServices/Game';

const getRecentProjectFiles = (count: number) =>
  new Array<number>(count).fill(0).map((_, index) => ({
    fileMetadata: {
      fileIdentifier: `Users/me/Desktop/AirStudio/project ${index}`,
...
const getPartiallySavedRecentProjectFiles = (count: number) =>
  new Array<number>(count).fill(0).map((_, index) => ({
    fileMetadata: {
      fileIdentifier: `Users/AirStudio/project ${index}`,
      lastModifiedDate:
        index % 3 === 0
...
export default {
  ...
  decorators: [AirStudioJsInitializerDecorator, inAppTutorialDecorator],
};
