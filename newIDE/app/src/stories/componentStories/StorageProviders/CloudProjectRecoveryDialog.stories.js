...
import { fakeSilverAuthenticatedUserWithCloudProjects } from '../../../fixtures/AirStudioServicesTestData';
import {
  AirStudioProjectApi,
...
export const Default = (): React.Node => {
  ...
  projectServiceMock
    .onGet(`${AirStudioProjectApi.baseUrl}/project/${projectId}/version`)
    .reply(200, versions)
    .onAny()
    .reply(config => {
  ...
  createZipWithSingleTextFile(JSON.stringify(emptyGameContent)).then(
    zipFile => {
      ...
      resourcesMock
        .onGet(
          `${AirStudioProjectResourcesStorage.baseUrl}/${projectId}/versions/${
            versions[1].id
          }.zip`
        )
        .reply(200, zipFile, {
