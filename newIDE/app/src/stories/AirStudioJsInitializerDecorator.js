...
const AirStudioJsInitializer = ({ children }: AirStudioJsInitializerProps) => {
  ...
  React.useEffect(() => {
    ...
    console.info(
      'Loading AirStudio.js and creating test extensions/test project...'
    );
    GD_STARTUP_TIMES.push(['initializeAirStudioJsCall', performance.now()]);
    initializeAirStudioJs().then(gd => {
      GD_STARTUP_TIMES.push(['initializeAirStudioJsDone', performance.now()]);
      ...
      console.info(
        'Done loading AirStudio.js and created test extensions/test project.'
      );
  ...
  return <div>Loading AirStudio.js, test extensions and test project...</div>;
};

const libGDDecorator: StoryDecorator = (Story, context) => (
  <AirStudioJsInitializer>{() => <Story />}</AirStudioJsInitializer>
);
