...
import { hasValidSubscriptionPlan } from '../Utils/AirStudioServices/Usage';
        ...
        <ColumnStackLayout expand noMargin>
          ...
          <ColumnStackLayout noMargin>
            <ResponsiveLineStackLayout noResponsiveLandscape noMargin>
              <Column expand noMargin justifyContent="center">
                <Checkbox
                  label={
                    <Trans>
                      Display AirStudio logo at startup (in exported game)
                    </Trans>
                  }
                  checked={loadingScreen.isAirStudioLogoShownDuringLoadingScreen()}
                  onCheck={(e, checked) => {
                    if (
                      !checked &&
                      !watermark.isAirStudioWatermarkShown() &&
                      subscriptionChecker.current &&
                      !subscriptionChecker.current.checkUserHasSubscription()
                    ) {
                      // If user wants to deactivate AirStudio splash screen although
                      // watermark is hidden, we don't allow it if they have no subscription.
                      return;
                    }
                    loadingScreen.showAirStudioLogoDuringLoadingScreen(checked);
                    onUpdate();
                  }}
                />
              </Column>
              <Column expand noMargin justifyContent="center">
                <SelectField
                  fullWidth
                  floatingLabelText={<Trans>AirStudio logo style</Trans>}
                  value={loadingScreen.getAirStudioLogoStyle()}
                  onChange={(e, i, newAirStudioLogoStyle: string) => {
                    const currentAirStudioLogoStyle = loadingScreen.getAirStudioLogoStyle();
                    if (currentAirStudioLogoStyle === newAirStudioLogoStyle)
                      return;
                    loadingScreen.setAirStudioLogoStyle(newAirStudioLogoStyle);
                    onUpdate();
                  }}
                  disabled={
                    !loadingScreen.isAirStudioLogoShownDuringLoadingScreen()
                  }
                >
            ...
            <ResponsiveLineStackLayout noResponsiveLandscape noMargin>
              <Column expand noMargin justifyContent="center">
                <Checkbox
                  label={
                    <Trans>
                      Display AirStudio watermark after the game is loaded (in
                      exported game)
                    </Trans>
                  }
                  checked={watermark.isAirStudioWatermarkShown()}
                  onCheck={(e, checked) => {
                    if (
                      !checked &&
                      !loadingScreen.isAirStudioLogoShownDuringLoadingScreen() &&
                      ...
                      // If user wants to deactivate watermark although AirStudio splash
                      // screen is hidden, we don't allow it if they have no subscription.
                      return;
                    }
                    watermark.showAirStudioWatermark(checked);
                    if (checked) {
              ...
              <Column expand noMargin justifyContent="center">
                <SelectField
                  fullWidth
                  floatingLabelText={
                    <Trans>AirStudio watermark placement</Trans>
                  }
                  value={watermark.getPlacement()}
                  onChange={(e, i, newPlacement: string) => {
                    const currentAirStudioLogoStyle = loadingScreen.getAirStudioLogoStyle();
                    if (currentAirStudioLogoStyle === newPlacement) return;
                    watermark.setPlacement(newPlacement);
                    onUpdate();
                  }}
                  disabled={!watermark.isAirStudioWatermarkShown()}
                >
            ...
            {!hasValidSubscription && (
              <GetSubscriptionCard
                subscriptionDialogOpeningReason="Disable AirStudio splash at startup"
                recommendedPlanId="AirStudio_silver"
                placementId="AirStudio-branding"
              >
                <Text>
                  <Trans>
                    Get a silver or gold subscription to disable AirStudio
                    branding.
                  </Trans>
                </Text>
              </GetSubscriptionCard>
          ...
          <SemiControlledTextField
            ...
            onChange={newValue => {
              ...
              if (
                ...
                // If users want to reduce AirStudio splash screen although
                // watermark is hidden, we don't allow it if they have no subscription.
                return;
              }
          ...
          <SubscriptionChecker
            ...
            mode="mandatory"
            id="Disable AirStudio splash at startup"
            title={<Trans>Disable AirStudio splash at startup</Trans>}
            placementId="AirStudio-branding"
          />
        </ColumnStackLayout>
      )}
