...
export type airStudioTheme = typeof DefaultLightTheme.airStudioTheme;
type FullTheme = {| airStudioTheme: airStudioTheme, muiTheme: Object |};
const defaultThemeName = 'AirStudio Light Blue';
...
|}): FullTheme {
  ...
  const { airStudioTheme, muiThemeOptions } = theme;
  return {
    airStudioTheme,
...
export function getThemeWindowBackgroundColor(): string {
  try {
    ...
    if (values && values.themeName) {
      ...
      return theme.airStudioTheme.surface.window.backgroundColor;
    }
  } catch {}
  return '#f0f0f0';
}
