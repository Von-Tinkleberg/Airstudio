...
import AirStudioThemeContext from './AirStudioThemeContext';
...
export const FullThemeProvider = ({
  ...
  const theme = React.useMemo(
    () => {
      ...
      try {
        ...
        body.className = fullTheme.airStudioTheme.uiRootClassName;
      } catch (error) {
  ...
  return (
    <AirStudioThemeContext.Provider value={theme.airStudioTheme}>
      <StylesProvider jss={portalJss || jss}>
        <ThemeProvider theme={muiTheme}>{children}</ThemeProvider>
      </StylesProvider>
    </AirStudioThemeContext.Provider>
  );
};
