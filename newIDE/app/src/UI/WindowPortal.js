 ...
 * so that Material-UI and AirStudio themes work correctly.
 *
...
const WindowPortal = ({
  ...
  React.useEffect(() => {
    ...
    const targetId = `AirStudioWindowPortal${++popOutCounter}`;
    ...
    // Copy the body class from the parent (used for AirStudio theme).
    if (document.body) {
    ...
    // cleanup in the useEffect return function never fires â€” leaving "dead"
  ...
  //    derives ownerDocument from the container â€” must be the external window's body.
  // 3. ClickAwayListener (used by InlinePopover) attaches event listeners to
  //    ownerDocument of the portal content â€” must be the external window's document.
...
function copyStyleElementToDocument(
  ...
  if (sourceStyleEl.textContent) {
    ...
    try {
      ...
      // Cross-origin stylesheets can't be read â€“ skip silently.
    }
  }
