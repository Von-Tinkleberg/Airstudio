...
import { type UserPublicProfileByIds } from '../Utils/AirStudioServices/User';
...
const StatusIndicator = ({ status }: {| status: VersionRestoringStatus |}) => {
  ...
  const backgroundColor = getStatusColor(airStudioTheme, status);
...
const useBorder = (
  ...
  return { border: `1px solid ${airStudioTheme.statusIndicator.error}` };
};
