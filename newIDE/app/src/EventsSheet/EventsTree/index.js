  ...
  return (
    <SortableEventsTree
      className={`${eventsTree} ${
        airStudioTheme.palette.type === 'light' ? 'light-theme' : 'dark-theme'
      } ${className}`}
      {...otherProps}
    />
  );
...
const EventsTree: React.ComponentType<{
  ...
  const _onEndDrag = React.useCallback(
    () => {
      ...
      // draggedNode â€” so checking the value makes it see draggedNode=null (stale
      // closure) and skips the cleanup. Calling unconditionally is safe: React
      // deduplicates the nullâ†’null state update, and _restoreFoldedNodes is a
      ...
      const tryScrollToElement = ({
        ...
        requestAnimationFrame(() => {
          ...
          if (element) {
            ...
            // Element not in DOM â€” the row may not be rendered by the virtual
  ...
  React.useLayoutEffect(() => {
    ...
    // â€” not just changes to individual event heights. Scroll-triggered
