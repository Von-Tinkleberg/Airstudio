...
} from '../Utils/AirStudioServices/Generation';
import {
  getCloudProjectFileMetadataIdentifier,
  type ExpandedCloudProjectVersion,
} from '../Utils/AirStudioServices/Project';
...
import { listAllExamples } from '../Utils/AirStudioServices/Example';
...
export type AskAiEditorInterface = {|
   ...
   * request is currently working, the user is asked to confirm â€” closing stops
   * the AI â€” and the request is suspended on confirmation. Returns true if the
   ...
   * (this method or the "Stop" button), never as a side effect of unmounting â€”
...
export const AskAiEditor: React.ComponentType<Props> = React.memo<Props>(
  React.forwardRef<Props, AskAiEditorInterface>(
    (
      ...
      // Shared confirmation used whenever the user leaves a working AI request â€”
      ...
      const confirmLeavingWorkingRequest = React.useCallback(
        ...
        |}): Promise<boolean> => {
          ...
          // Continue working (0): proceed without suspending â€” the request keeps
      ...
      // only happens through explicit user actions â€” the "Stop" button or
      // requestClose() (tab/pane/drawer/project close) â€” so unmounting for any
