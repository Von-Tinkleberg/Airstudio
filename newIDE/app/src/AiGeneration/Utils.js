 ...
 * the agent's name â€” the label of the call that launched it in the parent
  ...
  const onProcessFunctionCalls = React.useCallback(
    async (
      ...
      try {
        ...
        // results â€” we don't want to re-populate the cleared results or send
 ...
 *
 * One-shot fetch only â€” the polling/activation pipeline remains responsible
  ...
  React.useEffect(
    () => {
      if (prevProjectRef.current !== project) {
        ...
        // is first opened from scratch (null â†’ project), e.g. when the AI
        // creates a new project â€” we want to keep the in-progress request.
