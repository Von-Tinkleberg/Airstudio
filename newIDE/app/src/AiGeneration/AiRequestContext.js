  ...
  return {
    ...
    addEditorFunctionCallResults: React.useCallback(
      (
        ...
        // to any code that calls getEditorFunctionCallResults â€” even within
...
export const useAiRequestsStorage = (): AiRequestStorage => {
  ...
  // immediately consistent â€” even within the same async tick.
...
export type AiRequestContextState = {|
   ...
   * when the Ask AI editor is closed, since the request lives in this provider â€”
...
export const AiRequestProvider = ({
  ...
  // to the watch loop, so polling can be paused while the prompt is shown â€” the
      ...
      await Promise.all(
        statusOnlyIds.map(async aiRequestId => {
          ...
          if (newStatus !== currentRequest.status) {
            // Status changed â€” full fetch immediately to pick up new messages.
            sawChangeThisTick = true;
