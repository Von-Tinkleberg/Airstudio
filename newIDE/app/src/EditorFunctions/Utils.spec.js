...
describe('getObjectSizeInfo', () => {
  ...
  describe('Sprite', () => {
    it('returns texture dimensions Ã— preScale with origin and center when texture is valid', () => {
      ...
      // Leave center as default (isDefaultCenterPoint = true) â†’ center = texture dimensions / 2
    ...
    it('uses assetShortHeader dimensions when provided (asset store case)', () => {
      ...
      animation.setDirectionsCount(1);
      const sprite = new gd.Sprite();
      // Image name not in mock loader â€” but assetShortHeader provides the dimensions
      sprite.setImageName('UnknownImage');
      animation.getDirection(0).addSprite(sprite);
      spriteConfig.getAnimations().addAnimation(animation);
