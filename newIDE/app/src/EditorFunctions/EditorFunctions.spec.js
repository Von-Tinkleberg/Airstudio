// @flow
import { fakeAssetShortHeader1 } from '../fixtures/AirStudioServicesTestData';
...
describe('editorFunctions', () => {
  ...
  describe('change_object_property', () => {
    ...
    it('handles various property types: resource, non-existing, boolean, and number with sanitization warning', async () => {
      ...
      expect(result.message).toMatchInlineSnapshot(`
        ...
        \\"shadowBlurRadius\\" on \\"MyTextObject\\" = \\"20.41\\", but requested \\"20.41 Ã— 50\\" looks multi-dimensional; only a single number is allowed."
      `);
  ...
  describe('put_2d_instances (new-instance attributes in message)', () => {
    ...
    it('includes size/rotation/opacity/z-order in the new-instance message', async () => {
      ...
      expect(result.message).toEqual(
        expect.stringMatching(
          /Created 2 new instances of object "Player" \(ids: [0-9a-f-]{10}, [0-9a-f-]{10}\) using point brush at 50, 60 on the base layer \(""\) \(size 64x64, rotation 45Â°, opacity 128\/255, z-order 5, origin at this position, each occupies X 50 to 114, Y 60 to 124\)\./
        )
      );
    });
  });
  ...
  describe('put_2d_instances (brush placement positions)', () => {
    ...
    // fall back to the scene center â€” a major source of instances duplicated
    ...
    it('fails when erasing an unknown instance id (none found, nothing changed)', async () => {
      // Bypass the `putInstances` helper, which asserts success â€” here we
  ...
  describe('put_3d_instances (new-instance attributes in message)', () => {
    ...
    it('includes size and rotation in the new-instance message', async () => {
      ...
      expect(result.message).toEqual(
        expect.stringMatching(
          /Created 1 new instance of object "Player" \(id: [0-9a-f-]{10}\) using point brush at 10, 20, 30 on the base layer \(""\) \(size 8x16x24, rotation \(15Â°, 30Â°, 45Â°\), origin at this position, each occupies X 10 to 18, Y 20 to 36, Z 30 to 54\)\./
        )
      );
    });
  });
  ...
  describe('change_scene_properties_layers_effects_groups (echo new values)', () => {
    ...
    it('keeps spaces and special characters when renaming a scene', async () => {
      const result = await editorFunctions.change_scene_properties_layers_effects_groups.launchFunction(
        {
          ...
          args: {
            scene_name: 'TestScene',
            changed_properties: [
              { property_name: 'name', new_value: 'Templo das EstaÃ§Ãµes' },
            ],
          },
        }
      );
      ...
      expect(result.success).toBe(true);
      expect(result.message).toContain(
        'Renamed scene "TestScene" to "Templo das EstaÃ§Ãµes"'
      );
      expect(project.hasLayoutNamed('Templo das EstaÃ§Ãµes')).toBe(true);
      expect(project.hasLayoutNamed('Templo_das_EstaÃ§Ãµes')).toBe(false);
    });
