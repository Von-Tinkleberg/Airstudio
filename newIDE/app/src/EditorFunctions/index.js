...
import { type AiGeneratedEvent } from '../Utils/AirStudioServices/Generation';
import {
...
import { type AssetShortHeader } from '../Utils/AirStudioServices/Asset';
...
const getAvailablePropertyNamesText = (
  ...
  return ` Available properties: ${names.slice(0, maxCount).join(', ')}${
    names.length > maxCount ? `, â€¦ (${names.length - maxCount} more)` : ''
  }.`;
};
  ...
  if (foundProperty.getType().toLowerCase() === 'boolean') {
    ...
    if (requestedNewValueAsBooleanString !== actualNewValue) {
      propertyWarnings.push(
        `${propertyNameWithLocation}: actual "${actualNewValue}" â‰  requested "${requestedNewValueAsBooleanString}".`
      );
    }
  } else {
    if (
      ...
      if (foundProperty.getType().toLowerCase() === 'number') {
        const sizeLikeRegex = /^\s*([+-]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)\s*([,;xX*Ã—])\s*([+-]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)\s*(?:\2\s*([+-]?(?:\d+(?:\.\d+)?|\.\d+)(?:[eE][+-]?\d+)?)\s*)?$/;
        ...
        propertyWarnings.push(
          `${propertyNameWithLocation}: actual "${actualNewValue}" â‰  requested "${requestedNewValue}".`
        );
      }
    }
  }
...
const getShortMeasurementUnit = (
  ...
  try {
    ...
    for (let i = 0; i < elementsCount; i++) {
      ...
      if (!symbol) continue;
      shortLabel +=
        (shortLabel ? 'Â·' : '') + symbol + (power === 1 ? '' : `^${power}`);
    }
  } catch (_) {
...
const applyObjectPropertyChange = ({
  ...
  // stored property changed â€” so `verifyPropertyChange` cannot catch it.
 ...
 * like a layer does â€” effects are only listed if the object type supports
  ...
  launchFunction: async ({
    ...
    if (brush_kind === 'erase') {
      ...
      if (brush_kind === 'none' && parsedBrushPosition) {
        return makeGenericFailure(
          'The "none" brush never moves instances: remove `brush_position`, or use the "point" brush to move (it sets the exact position of every matched instance â€” one call per target position).'
        );
      }
      ...
      if (newInstancesCount > 0) {
        ...
        if (instancesRotation !== null)
          attrs.push(`rotation ${instancesRotation}Â°`);
        if (instancesOpacity !== null)
      ...
      if (notFoundExistingInstanceIds.size > 0) {
        ...
        // real error signal instead of a misleading success â€” a success here
        ...
        if (!hasMutationParams && !hasPositionBrush) {
          return makeGenericFailure(
            `Matched ${matchedCount} existing instance${
              matchedCount > 1 ? 's' : ''
            } but no change was requested â€” provide a value to modify, or use the "point" brush with \`brush_position\` to move.`
          );
        }
...
const put3dInstances: EditorFunction = {
  ...
  launchFunction: async ({
    ...
    if (brush_kind === 'erase') {
      ...
      if (brush_kind === 'none' && parsedBrushPosition) {
        return makeGenericFailure(
          'The "none" brush never moves instances: remove `brush_position`, or use the "point" brush to move (it sets the exact X, Y and Z of every matched instance â€” one call per target position).'
        );
      }
      // After the guard, a missing position can only happen when nothing is
      // created nor moved ("none" brush only): the fallback is never used.
      ...
      if (newInstancesCount > 0) {
        ...
        if (instancesRotationArray && instancesRotationArray.length >= 3)
          attrs.push(
            `rotation (${instancesRotationArray[0]}Â°, ${
              instancesRotationArray[1]
            }Â°, ${instancesRotationArray[2]}Â°)`
          );
      ...
      if (rotatedCount > 0 && instancesRotationArray) {
        changes.push(
          ...
          }Â°, ${instancesRotationArray[1]}Â°, ${instancesRotationArray[2]}Â°).`
        );
      }
  ...
  if (newlyCreatedEffect) {
    ...
    if (gd.MetadataProvider.isBadEffectMetadata(effectMetadata)) {
      ...
      if (
        ...
        warnings.push(
          `"${newlyCreatedEffect.getName()}" only works in 3D, but ${targetLabel} is restricted to 2D â€” it may have no visible effect.`
        );
      } else if (
        ...
        warnings.push(
          `"${newlyCreatedEffect.getName()}" only works in 2D, but ${targetLabel} is restricted to 3D â€” it may have no visible effect.`
        );
      }
    }
  }
...
const readFullDocs: EditorFunction = {
  ...
  launchFunction: async ({ args }) => {
    return makeGenericFailure(
      `Unable to read full documentation - continue with your existing AirStudio knowledge.`
    );
  },
  modifiesProject: false,
};
...
const searchDocs: EditorFunction = {
  renderForEditor: ({ args }) => {
    return {
      text: <Trans>Search AirStudio documentation.</Trans>,
    };
  },
  launchFunction: async ({ args }) => {
    return makeGenericFailure(
