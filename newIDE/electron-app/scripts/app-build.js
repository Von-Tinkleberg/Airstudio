...
if (!shell.test('-f', './node_modules/.bin/electron-builder')) {
  shell.echo('âš ï¸ Please run npm install in electron-app folder');
  shell.exit(1);
}
...
const checkLibGDjsSize = () => {
  ...
  return new Promise(resolve => {
    fs.stat(path.join(appPublicPath, 'libGD.js'), (err, stats) => {
      ...
      shell.echo(`âœ… libGD.js size seems correct (${sizeInMiB.toFixed(2)}MiB)`);
...
checkLibGDjsSize().then(() => {
  ...
  if (shell.cp('-r', '../app/build/*', 'app/www').code !== 0) {
    shell.echo(
      `âŒ Copy from "../app/build" to Electron's "app/www" folder failed.`
    );
    shell.exit(1);
  }
});
