...
const CLI_SYMLINK_NAME = 'AirStudio';
  ...
  return new Promise(resolve => {
    child_process.execFile(
      ...
      error => {
        ...
        resolve({
          status: 'success',
          message: `AirStudio added to PATH. Open a new terminal to use the CLI.`,
        });
      }
    );
  });
    ...
    const onDone = error => {
      if (error) {
        if (isElevationCancelled(error)) {
          resolve({
            status: 'error',
            message: 'AirStudio CLI installation was cancelled.',
          });
          return;
        }
        resolve({
      ...
      resolve({
        status: 'success',
        message: `AirStudio CLI available as "${CLI_SYMLINK_NAME}".`,
      });
    };
...
/**
 * Register the AirStudio binary in the system PATH so the CLI works anywhere.
