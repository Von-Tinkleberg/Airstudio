...
log.info('AirStudio Electron app starting...');
...
if (process.platform === 'win32') {
  app.setAppUserModelId('AirStudio.ide');
}
...
// Function to create a new AirStudio window
function createNewWindow(windowArgs = args) {
  ...
  if (windowCounter > 0) {
    ...
    options.webPreferences.partition = `persist:AirStudio-window-${windowCounter}`;
    log.info(
      `Creating window #${windowCounter} with partition: persist:AirStudio-window-${windowCounter}`
    );
  } else {
    log.info(
    ...
    app.setUserTasks([
      {
        ...
        iconIndex: 0,
        title: 'New window',
        description: 'Open a new AirStudio window',
      },
    ]);
  }
